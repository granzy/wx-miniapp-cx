//logs.js
const util = require('../../utils/util.js')
var qcloud = require('../../vendor/wafer2-client-sdk/index')
var config = require('../../config')

Page({
  data: {

    id:'',
    idCardNo: '',
    carNo: '',
    phoneNo: '',
    // 身份证识别
    imgUrl: '',
    idCardInfo: {},
    showResult: false
  },
  doIdCardIdentify: function () {
    var that = this

    that.setData({
      showResult: false
    })

    // 选择图片和上传图片
    this._chooseImgAndUpload(
      config.service.ciUrl + '?action=idcard',
      // 上传图片之前
      function (filePath) {
        that.setData({
          imgUrl: filePath
        })
      },
      // 调用成功
      function (res) {
        util.showSuccess('识别成功')
        var data = JSON.parse(res.data)

        if (data.code !== 0) {
          util.showModel('识别失败')
          return
        }

        var info = data.data[0]

        if (info.code !== 0) {
          util.showModel('识别失败' + info.message)
          return
        }

        that.setData({
          showResult: true,
          idCardInfo: info.data,
          idCardNo: info.data.id
        })
      },
      // 调用失败
      function (e) {
        util.showModel('识别失败' + e.message)
      }
    )
  },
  /**
   * 统一封装选择图片和上传图片的 API
   * @param {Function} beforUpload 开始上传图片之前执行的函数
   * @param {Function} success     调用成功时执行的函数
   * @param {Function} fail        调用失败时执行的函数
   */
  _chooseImgAndUpload(url, beforUpload, success, fail) {
    var that = this

    // 选择图片
    wx.chooseImage({
      count: 1,
      sizeType: ['compressed'],
      sourceType: ['album', 'camera'],
      success: function (res) {
        util.showBusy('正在识别')
        var filePath = res.tempFilePaths[0]

        beforUpload(filePath)

        // 上传图片
        wx.uploadFile({
          url,
          filePath: filePath,
          name: 'file',
          success: success,
          fail: fail
        })
      },
      fail: fail
    })
  },
  onLoad: function (options) {

    if(options.id){
      this.setData({
        id:options.id,
        idCardNo:options.idcardno,
        carNo:options.carno,
        phoneNo:options.phoneno
      });
    }

  },
  navigateToCarInfo: function (e) {
    if (this.data.idCardNo == '') {
      wx.showModal({
        title: '提示',
        content: '身份证号不能为空',
      })
    } else if (this.data.carNo == '') {
      wx.showModal({
        title: '提示',
        content: '车牌号不能为空',
      })
    } else if (this.data.phoneNo == '') {
      wx.showModal({
        title: '提示',
        content: '手机号不能为空',
      })
    } else {
      let url = e.currentTarget.dataset.url;
      util.showBusy('请求中...')
      let openid = wx.getStorageSync('openid');
      var that = this
      wx.request({
        url: `${config.service.host}/weapp/insertCarInfo?idCardNo=` + this.data.idCardNo + '&carNo=' + this.data.carNo + '&phoneNo=' + this.data.phoneNo + '&openid='+openid,
        success(result) {
          wx.setStorage({
            key: 'userCarInfo',
            data: {
              id:result.data.data.id,
              idCardNo: that.data.idCardNo,
              carNo: that.data.carNo,
              phoneNo: that.data.phoneNo
            },
          });
          wx.navigateTo({
            url: url
          });
        },
        fail(error) {
          util.showModel('请求失败', error);
          console.log('request fail', error);
        }
      })
    }
  },
  editCarInfo:function(){
    util.showBusy('请求中...')
    var that = this
    wx.request({
      url: `${config.service.host}/weapp/updateCarInfo`,
      method:'POST',
      data:{
        id: that.data.id,
        idcardno: that.data.idCardNo,
        carno: that.data.carNo,
        phoneno: that.data.phoneNo
      },
      success(result) {
        wx.showToast({
          title: '已完成',
          icon: 'success',
          duration: 3000
        });
      },
      fail(error) {
        util.showModel('请求失败', error);
        console.log('request fail', error);
      }
    })
  },
  bindIdcardInput: function (e) {
    this.setData({
      idCardNo: e.detail.value
    })
  },
  bindCarNoInput: function (e) {
    this.setData({
      carNo: e.detail.value
    })
  },
  bindPhoneNoInput: function (e) {
    this.setData({
      phoneNo: e.detail.value
    })
  }
})
